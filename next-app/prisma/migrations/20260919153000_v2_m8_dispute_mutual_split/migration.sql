...
-- M8 extends the M2 Escrow RELEASED primitive with one additional canonical
-- settlement posting shape. Full settlement remains the original two-entry
-- escrow_release. Mutual split is exactly three entries: one full debit from
-- Buyer locked and two Principal-available credits whose total equals Escrow.
...
BEGIN
  ...
  IF NEW."status" = 'released' THEN
    ...
    IF tx_status IS DISTINCT FROM 'posted'
       ...
       OR tx_reference_type NOT IN ('escrow_release', 'escrow_mutual_split')
       OR tx_reference_id IS DISTINCT FROM NEW."contractId" THEN
    ...
    IF (
         tx_reference_type = 'escrow_release'
         AND (
           entry_count <> 2
           OR matched_count <> 2
           OR debit_amount <> NEW."amount"
           OR credit_amount <> NEW."amount"
         )
       )
       OR (
         tx_reference_type = 'escrow_mutual_split'
         AND (
           entry_count <> 3
           OR matched_count <> 3
           OR debit_amount <> NEW."amount"
      ...
      RAISE EXCEPTION 'Escrow release must consume exactly the Escrow amount using the canonical settlement posting shape'
        USING ERRCODE = '23514';
    END IF;
  ELSE
    SELECT "status", "type", "referenceType", "referenceId"
