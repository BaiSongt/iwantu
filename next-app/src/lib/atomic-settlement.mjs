...
} from './ledger/credit-foundation.mjs';
import {
  LedgerPostingError,
...
export class AtomicSettlementError extends Error {
  constructor(code, message, details = undefined) {
    super(message);
    this.name = 'AtomicSettlementError';
    this.code = code;
...
function deny(code, message, details) {
  throw new AtomicSettlementError(code, message, details);
}

function nonEmpty(value, field) {
  if (typeof value !== 'string' || value.trim().length === 0) {
    deny('SETTLEMENT_INPUT_INVALID', `${field} must be a non-empty string`, { field });
  }
  return value.trim();
}
...
export function buildSettlementEvidence(input) {
  return canonicalize({
    protocol: 'iwantu.settlement.v0.1',
    ...
    acceptanceDecisionId: nonEmpty(input.acceptanceDecisionId, 'acceptanceDecisionId'),
    acceptanceDecisionHash: nonEmpty(input.acceptanceDecisionHash, 'acceptanceDecisionHash'),
    acceptanceSource: nonEmpty(input.acceptanceSource, 'acceptanceSource'),
...
export function hashSettlementEvidence(evidence) {
...
async function loadExistingSettlement(tx, contractId) {
  const rows = await tx.$queryRaw(Prisma.sql`
    SELECT * FROM "settlements" WHERE "contractId" = ${contractId}
  `);
  return rows[0] ?? null;
}
...
async function loadAcceptanceDecision(tx, contractId, deliveryId) {
  const rows = await tx.$queryRaw(Prisma.sql`
    SELECT *
    FROM "delivery_acceptance_decisions"
    WHERE "contractId" = ${contractId} AND "deliveryId" = ${deliveryId}
    LIMIT 1
  `);
  return rows[0] ?? null;
}
...
async function loadEscrowForUpdate(tx, escrowId, contractId) {
  const rows = await tx.$queryRaw(Prisma.sql`
    SELECT e.*, e."amount"::text AS "amountText", a."principalId" AS "buyerPrincipalId"
    FROM "escrows" e
...
async function loadBuyerLockedAccount(tx, buyerPrincipalId) {
  const rows = await tx.$queryRaw(Prisma.sql`
    SELECT *
    FROM "ledger_accounts"
    WHERE "principalId" = ${buyerPrincipalId}
...
async function requireActiveSupplier(tx, supplierPrincipalId, supplierAgentIdentityId) {
  const rows = await tx.$queryRaw(Prisma.sql`
    SELECT p."status" AS "principalStatus", a."status" AS "agentStatus", a."principalId"
    FROM "principals" p
    JOIN "agent_identities" a ON a."id" = ${supplierAgentIdentityId}
    WHERE p."id" = ${supplierPrincipalId}
    FOR SHARE OF p, a
  `);
  const row = rows[0];
  if (!row || row.principalId !== supplierPrincipalId) {
    deny('SETTLEMENT_SUPPLIER_BINDING_INVALID', 'Supplier Agent does not belong to Contract supplier', {
      supplierPrincipalId,
      supplierAgentIdentityId,
    });
  }
  if (row.principalStatus !== 'active' || row.agentStatus !== 'active') {
    deny('SETTLEMENT_SUPPLIER_INACTIVE', 'Settlement recipient must remain active', {
      supplierPrincipalId,
      supplierAgentIdentityId,
...
function assertExistingSettlement(existing, input) {
  if (existing.idempotencyKey !== input.idempotencyKey) {
    deny('SETTLEMENT_IDEMPOTENCY_CONFLICT', 'Contract already has a different terminal Settlement', {
...
function releasePostingInput({ contract, escrow, lockedAccount, supplierAvailableAccount, acceptance }) {
  ...
  return {
    ...
    idempotencyKey: `escrow:release:${contract.id}`,
    metadata: {
    ...
    entries: [
      ...
      {
        accountId: supplierAvailableAccount.id,
        side: 'credit',
...
async function performSettlement(tx, input, supplierAvailableAccount) {
  ...
  if (!contract) {
    deny('SETTLEMENT_CONTRACT_NOT_FOUND', 'Contract does not exist', { contractId: input.contractId });
  }

  const existing = await loadExistingSettlement(tx, input.contractId);
  if (existing) {
    assertExistingSettlement(existing, input);
    return { settlement: existing, replayed: true };
  }

  if (contract.lifecycleState !== 'acceptance_pending') {
    deny('SETTLEMENT_REQUIRES_ACCEPTANCE_PENDING', 'Settlement requires ACCEPTANCE_PENDING Contract', {
      contractId: contract.id,
      lifecycleState: contract.lifecycleState,
    });
  }
  if (!contract.escrowId) {
    deny('SETTLEMENT_ESCROW_NOT_BOUND', 'Contract does not have bound Escrow', { contractId: contract.id });
  }

  const delivery = await loadLatestDelivery(tx, contract.id);
  if (!delivery || delivery.effectiveContractHash !== contract.effectiveContractHash) {
    deny('SETTLEMENT_DELIVERY_BINDING_INVALID', 'Latest Delivery does not bind current Contract hash', {
      contractId: contract.id,
    });
  }
  ...
  if (!acceptance || acceptance.decision !== 'accept') {
    deny('SETTLEMENT_ACCEPTANCE_REQUIRED', 'Terminal Settlement requires ACCEPT or AUTO_ACCEPT evidence', {
      contractId: contract.id,
      deliveryId: delivery.id,
    });
  }
  if (
    acceptance.effectiveContractHash !== contract.effectiveContractHash
    || acceptance.deliveryHash !== delivery.deliveryHash
  ) {
    deny('SETTLEMENT_ACCEPTANCE_BINDING_INVALID', 'Acceptance evidence does not bind latest Delivery', {
      contractId: contract.id,
      deliveryId: delivery.id,
      acceptanceDecisionId: acceptance.id,
    });
  ...
  if (!escrow || escrow.status !== 'locked') {
    deny('SETTLEMENT_REQUIRES_LOCKED_ESCROW', 'Settlement requires Contract Escrow to be locked', {
      contractId: contract.id,
  ...
  await requireActiveSupplier(tx, contract.supplierPrincipalId, contract.supplierAgentIdentityId);
  const lockedAccount = await loadBuyerLockedAccount(tx, escrow.buyerPrincipalId);
  if (!lockedAccount) {
    deny('SETTLEMENT_LOCKED_ACCOUNT_MISSING', 'Buyer locked account is missing', {
      buyerPrincipalId: escrow.buyerPrincipalId,
    });
  }
  ...
  try {
    ledgerTransaction = await postLedgerTransactionInTransaction(
      ...
      releasePostingInput({
        ...
        escrow,
        lockedAccount,
        supplierAvailableAccount,
        acceptance,
      }),
    );
  } catch (error) {
    if (error instanceof LedgerPostingError) {
      deny('SETTLEMENT_LEDGER_POST_FAILED', 'Settlement ledger posting failed', {
  ...
  if (!ledgerTransaction.transactionHash) {
    deny('SETTLEMENT_LEDGER_HASH_MISSING', 'Posted settlement ledger transaction has no hash');
  }
  ...
  const evidence = buildSettlementEvidence({
    ...
    acceptanceDecisionId: acceptance.id,
    acceptanceDecisionHash: acceptance.decisionHash,
    acceptanceSource: acceptance.source,
  ...
  const allocation = {
    ...
    supplierAmount: escrow.amountText,
    buyerRefundAmount: '0',
  };
  ...
  await tx.$executeRaw(Prisma.sql`
    INSERT INTO "settlements" (
      ...
      "effectiveContractHash", "supplierPrincipalId", "supplierAgentIdentityId",
      "escrowId", "type", "ledgerTransactionId", "allocation", "settlementHash"
    ) VALUES (
      ${settlementId}, ${input.idempotencyKey}, ${contract.id}, ${delivery.id}, ${acceptance.id},
      ${contract.effectiveContractHash}, ${contract.supplierPrincipalId}, ${contract.supplierAgentIdentityId},
      ${escrow.id}, 'full_settlement', ${ledgerTransaction.id}, ${JSON.stringify(allocation)}::jsonb,
      ${settlementHash}
    )
  `);
  ...
  const settlement = await loadExistingSettlement(tx, contract.id);
  return { settlement, ledgerTransaction, replayed: false };
}

export async function settleAcceptedDelivery(prisma, input, options = {}) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    deny('SETTLEMENT_INPUT_INVALID', 'Settlement input must be an object');
  }
  const normalized = {
    contractId: nonEmpty(input.contractId, 'contractId'),
  ...
  const contractRows = await prisma.$queryRaw(Prisma.sql`
    SELECT "supplierPrincipalId" FROM "contracts" WHERE "id" = ${normalized.contractId}
  `);
  const supplierPrincipalId = contractRows[0]?.supplierPrincipalId;
  if (!supplierPrincipalId) {
    deny('SETTLEMENT_CONTRACT_NOT_FOUND', 'Contract does not exist', { contractId: normalized.contractId });
  }
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
    try {
      return await prisma.$transaction(
        (tx) => performSettlement(tx, normalized, supplierAvailableAccount),
      ...
      if (isSerializationFailure(error)) {
        deny('SETTLEMENT_CONCURRENCY_RETRY_EXHAUSTED', 'Settlement concurrency retries exhausted', {
